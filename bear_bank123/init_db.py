...
def init_db():
    ...
    # Inserir dados fictícios de usuários
    cursor.execute("""
    ...
    VALUES
    ('1234', '56789', '0', 'senha123', 'João Seguro', '12345678901', '61999999999', 'joao@exemplo.com', 'Rua das Flores', '123', 'Brasília', 'DF', 1000.00, 500.00),
    ('1234', '54123', '1', 'senha456', 'Maria Seguro', '98765432100', '61888888888', 'maria@exemplo.com', 'Av. Paulista', '456', 'São Paulo', 'SP', 2000.00, 1000.00)
    """)
    ...
    CREATE PROCEDURE sp_get_user_and_transactions (
        IN p_user_id INT,
        IN p_date_filter DATE
    )
    BEGIN
        -- Informações do usuário
        SELECT nome_completo, agencia, conta, digito
        FROM usuarios
        WHERE id_usuario = p_user_id;

        -- Transações do usuário com filtro de data
        IF p_date_filter IS NOT NULL THEN
            SELECT DISTINCT t.data_transacao AS date, 
                   tt.descricao AS transaction_type,
                   t.valor AS amount, 
                   t.origem AS origin,
                    t.destino AS destination
            FROM transacoes t
            JOIN tipos_transacoes tt ON t.tipo_transacao_id = tt.id_tipo
            WHERE t.id_usuario = p_user_id AND DATE(t.data_transacao) = p_date_filter
